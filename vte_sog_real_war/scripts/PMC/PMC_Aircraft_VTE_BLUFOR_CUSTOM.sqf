...
_PMC_Aircraft_VTE_BLUFOR =
{
	...
	if (pmc_debug) then { hint format["_targetpoint: %1", _targetpoint]; };
...
//	_vcl = _myVec createVehicle (getPosASL pmc_blufor_start_1);
	_vcl = createVehicle [_myVec, (getPosASL _respawnpoint), [], 0, "FLY"];
	_grp = objNull;
	_grp = createGroup west;
	waitUntil {!(isNull _grp)};

	"vte_acpilot" createUnit [(getPosASL _respawnpoint), _grp, "", 1, "SERGEANT"];
...
while {PMC_Aircraft_VTE_BLUFOR_running} do
{
	...
	_vcl = [_respawnpoint, _targetpoint] call _PMC_Aircraft_VTE_BLUFOR;
